		...
		Pass
		{
			CGPROGRAM
				...
				fixed4 frag (v2f i) : COLOR
				{
					...
					//float4 mc = (tex + (tex2D(_MatCap, worldNorm.xy * 0.5 + 0.5) * unity_ColorSpaceDouble) - 1.0);
					fixed4 mc = tex2D(_MatCap, worldNorm.xy * 0.5 + 0.5);
			#else
					half2 capCoord = half2(dot(i.c0, normals), dot(i.c1, normals));
					//float4 mc = (tex + (tex2D(_MatCap, capCoord*0.5 + 0.5) * unity_ColorSpaceDouble) - 1.0);
					fixed4 mc = tex2D(_MatCap, capCoord.xy * 0.5 + 0.5);
			#endif
