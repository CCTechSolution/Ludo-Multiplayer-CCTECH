...
public class ChallengeNotification : BasePopup
{
    ...
    {
        Events.RequestChallengeNotification += OnShow;
    }
    ...
    void OnShow(PlayerFriend _mFriend, int _mRegion, GameMode gameMode)
    {
        mFriend = _mFriend;
        mRegion = _mRegion;
    ...
    {
        string roomID = string.Format("CH-{0}", mFriend.mId);
        ...
        GameManager.Instance.Mode = GameManager.Instance.Mode;
        Events.RequestGameMatch.Call(roomID);
        Debug.Log("roomID= " + roomID);
      
       PlayFabManager.Instance.AcceptChallenge(mFriend, roomID, mRegion, (int)GameManager.Instance.Mode);
        HideNotification();
    }
