...
public class FacebookFriendsMenu : MonoBehaviour
{
    ...
    {
        //playersAvatars = StaticDataController.Instance.mConfig.customAvatars;////GameObject.Find("StaticGameVariablesContainer").GetComponent<StaticGameVariablesController>().avatars;
    ...
    public void updateFriendStatus(int status, string id)
    {
        foreach (GameObject friend in friendsObjects)
        {
            ...
            {
                if (status == 2/*ChatUserStatus.Online*/)
                {
                ...
                else if (status == 0/*ChatUserStatus.Offline*/)
                {
        ...
        PlayFabClientAPI.GetUserData(getdatarequest, (result) =>
        {
            ...
            if (data[Config.AVATAR_INDEX_KEY].Value.Equals(-1))
            {
                if (data.ContainsKey(Config.PLAYER_AVATAR_URL_KEY))
                {
                    ...
                    filterInputField.GetComponent<MonoBehaviour>().StartCoroutine(loadImage(data[Config.PLAYER_AVATAR_URL_KEY].Value, image));
                }
            }
            else
            {
                if (playersAvatars == null) Debug.Log("NULLLLL");
                image.sprite = playersAvatars[int.Parse(data[Config.AVATAR_INDEX_KEY].Value)];
            }
...
/**                        if (friend1[1].Equals("" + 0/*ChatUserStatus.Online*))
                            GameManager.Instance.facebookFriendsMenu.updateFriendStatus(2/*ChatUserStatus.Online*, friendID); */
                        break;
                    }
                }
    ...
    public void InviteFriend(string i)
    {
        ...
        to.Add(i);
        //FB.AppRequest(
        //    Config.facebookInviteMessage, to, null, null, null, null, null,
        ...
        //        {
        //            PlayFabManager.Instance.AddCoinsRequest(Config.rewardCoinsForFriendInvite);
        //        }
...
//        PlayFabManager.Instance.challengeFriend(id, GameManager.Instance.PayoutCoins + ";" + GameManager.Instance.PrivateRoomID);
    }
