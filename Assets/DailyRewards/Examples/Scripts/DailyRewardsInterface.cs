...
namespace NiobiumStudios
{
     ...
     **/
    public class DailyRewardsInterface : BasePopup
    {
        [Space(20)]
        ...
        {
            Events.RequestDailyRewards += OnShow;
        }
        ...
        private void OnInitialize(bool error, string errorMessage)
        {
            if (!error)
            {
                ...
                UpdateUI();
                canvas.SetActive(showWhenNotAvailable || (!showWhenNotAvailable && isRewardAvailable));
        ...
        IEnumerator AnimateCoins(int coins, Image pos)
        {
            yield return new WaitForSeconds(0.25f);
            Events.RequesCoinsColectionAnimation.Call(pos.rectTransform, coins, () => { panelReward.GetComponent<PlayTween>().AnimateHide(); });
        }
